...
public interface BookMapper {
    ...
    default Set<Author> authorsIdToAuthors(List<Long> authorsId) {
        return authorsId.stream().map(id -> Author.builder().id(id).build()).collect(Collectors.toSet());
    }
    default Set<Genre> genresIdToGenres(List<Long> genresId) {
        return genresId.stream().map(id -> Genre.builder().id(id).build()).collect(Collectors.toSet());
    }
}