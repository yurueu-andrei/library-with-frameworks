...
public abstract class BaseRepositoryTest {
    ...
    private void fillUsers() {
        users = new ArrayList<>() {{
            add(User.builder().id(1L).firstName("vlad").lastName("kaliaha").passportNumber("1111").email("email1").password("1111").address("address1").birthDate(LocalDate.of(2005, 6, 6)).status("ACTIVE").build());
            add(User.builder().id(2L).firstName("andrei").lastName("yurueu").passportNumber("2222").email("email2").password("123").address("address2").birthDate(LocalDate.of(2001, 6, 6)).status("ACTIVE").build());
